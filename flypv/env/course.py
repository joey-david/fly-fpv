...
@dataclass
class GateSpec:
    r_in: float = 0.030        # m, clear opening the fly must pass through
    ring_thickness: float = 0.006
    spacing: tuple[float, float] = (0.16, 0.26)   # m between consecutive gates
    turn: float = 0.9          # rad, max heading change per gate
    climb: float = 0.45        # rad, max pitch change per gate
    altitude: tuple[float, float] = (0.10, 0.55)  # m above the floor
...
class Course:
    ...
    def to_dict(self):
        return dict(center=self.center.tolist(), normal=self.normal.tolist(),
                    r_in=self.r_in.tolist(), r_out=self.r_out.tolist(),
                    start_pos=self.start_pos.tolist(), start_dir=self.start_dir.tolist())


def make_course(n_gates: int = 8, spec: GateSpec | None = None, seed: int | None = None,
                difficulty: float = 0.5) -> Course:
    ...
    for _ in range(n_gates):
        ...
        d = np.array([np.cos(pitch) * np.cos(heading),
